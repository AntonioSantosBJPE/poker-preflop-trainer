...
| D-24 | [error-messages-portuguese] Manter fallback `ipcErrorMessage` em vez de mensagens hardcoded nos catch blocks                                                          | Mensagens reais do main process são mais descritivas que hardcoded genéricos                                   | 2026-05-06 |
| D-25 | [error-messages-portuguese] Adicionar ErrorBoundary global no App.tsx                                                                                                 | Erros não-capturados no renderer devem mostrar feedback amigável em PT                                         | 2026-05-06 |
| D-26 | [error-messages-portuguese] Sanitizar erros IPC em 2 camadas: preload (sanitizeIpcError) + renderer (ipcErrorMessage com regex)                                       | Electron pode prefixar erros com 'Error invoking remote method'; dupla sanitização como defesa em profundidade | 2026-05-06 |
...
| error-messages-portuguese | 2026-05-06 | Design/Tasks — cobertura de testes detalhada |
...
- [error-messages-portuguese] Análise exaustiva revelou que a codebase já está **quase 100% em português** nas mensagens ao usuário. A percepção inicial de "mensagens em inglês" era exagerada — o único ponto crítico é `'Database not initialized'` em `db/client.ts`. O foco real deve ser: (1) substituir fallbacks hardcoded por `ipcErrorMessage`, (2) melhorar mensagens genéricas no main process, (3) adicionar ErrorBoundary global.
- [error-messages-portuguese] Electron (v41) pode prefixar erros de IPC com `Error invoking remote method '<channel>': <message>`. Este prefixo vaza detalhes internos (nome do canal) e precisa ser sanitizado antes de chegar ao usuário. A solução escolhida é sanitizar em 2 camadas: preload (sanitizeIpcError) + renderer (ipcErrorMessage com regex).
