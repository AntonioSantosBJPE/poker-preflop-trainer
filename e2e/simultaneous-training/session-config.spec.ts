...
test.describe('Treino simultâneo — configuração', () => {
  test('E2E-MT-03: inicia com 2 mesas', async ({ appPage }) => {
    ...
    const sessionIds = await startSimultaneousViaApi(appPage, { groupName, situationName, tableCount: 2 })
  ...
  test('E2E-MT-03: inicia com 3 mesas', async ({ appPage }) => {
    ...
    await goToSimultaneousStep2(appPage, groupName)
    const sessionIds = await startSimultaneousViaApi(appPage, { groupName, situationName, tableCount: 3 })
  ...
  test('E2E-MT-03: inicia com 4 mesas', async ({ appPage }) => {
    ...
    await goToSimultaneousStep2(appPage, groupName)
    const sessionIds = await startSimultaneousViaApi(appPage, { groupName, situationName, tableCount: 4 })
